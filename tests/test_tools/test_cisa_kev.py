...
def test_kev_detect_bad():
    parser = CisaKevParser()

    assert parser.detect("just some text") is False


def test_kev_node_parse():
    parser = CisaKevParser()
...
def test_kev_populate(use_graph):
    parser = CisaKevParser()

    input_data = parser._load_data(test_data)
    parser.parse_data(input_data, populate=True)

    assert CVE.get_count() == 5


@pytest.mark.slow
@pytest.mark.webtest
def test_kev_populate_web(use_graph):
    parser = CisaKevParser()
    ...
    cypher = """MATCH (cve:CVE) WHERE cve.cve_id = 'CVE-2019-0708' RETURN cve"""
